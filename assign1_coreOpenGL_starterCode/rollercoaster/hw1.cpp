...
void generateSpline(int spline_index) {//genrate a spline, including points, tangents, and normals, from splines[spline_index] to splinePoints
	...
	for (int n = 0; n < segment_num; n++) {
		...
		for (int j = 0; j < spline_interval_num; j++) {
			...
			splinePoints[index] = catmullRoll_point(controlpoint[0], controlpoint[1], controlpoint[2], controlpoint[3], delta_j);
			splineTangents[index] = catmullRoll_tangent(controlpoint[0], controlpoint[1], controlpoint[2], controlpoint[3], delta_j);
	...
	for (int i = 0; i < spline_interval_num * segment_num - 1; i++) {
		splinePoints_line[2 * i] = splinePoints_crosssection[4 * i];
		splinePoints_line[2 * i + 1] = splinePoints_crosssection[ 4 * i + 4];
	...
	*/
	...
	/*
	splinePoints_line = new glm::vec3[4 * (spline_interval_num * segment_num - 1)];
	splineColor_line = new glm::vec4[4 * (spline_interval_num * segment_num - 1)];
	...
	for (int i = 0; i < spline_interval_num * segment_num - 1; i++) {
		splinePoints_line[2 * (i + spline_interval_num * segment_num)] = splinePoints_crosssection[4 * i];
		splinePoints_line[2 * (i + spline_interval_num * segment_num )+ 1] = splinePoints_crosssection[4 * (i + 1)];
		splineColor_line[2 * (i + spline_interval_num * segment_num)] = { 1,1,1,1 };
		splineColor_line[2 * (i + spline_interval_num * segment_num )+ 1] = { 1,1,1,1 };
	}
	*/
	...
	for (int i = 0; i < 8 * (spline_interval_num * segment_num - 1); i++) {
